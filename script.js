...
const symbols = /[\r\n%#()<>?\[\\\]^{|}]/g;
const quotesInput = doc.querySelectorAll('.options__input');
...
function getResults() {
    ...
    const bgurl = `url(${quotes.level}${svgdata}${quotes.level})`;
...
contrastButton.onchange = function() {
    ...
    doc.body.backgroundColor = contrastButton.value;
}
...
function getQuotes() {
    ...
    return {
        level: externalQuotesValue === 'double' ? double : single,
